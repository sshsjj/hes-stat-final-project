...
```
```{r}
mean(player[player$PositionGroup == "FWD", ]$ValueNumeric_pounds)
mean(player[player$PositionGroup == "DEF", ]$ValueNumeric_pounds)
mean(player[player$PositionGroup == "MID", ]$ValueNumeric_pounds)
```
...
player1_without_PositionGroup <- subset(player1, select = -c(PositionGroup))
...
```{r}
player1 <- player
full_fit = lm(ValueNumeric_pounds ~.-WageNumeric_pounds-Release.Clause_pounds, data=player1)
summary(full_fit)
plot(full_fit)
```
...
```{r}
player1 <- player
full_fit = lm(ValueNumeric_pounds ~., data=player1)
player1
summary(full_fit)
plot(full_fit)
```
...
Not doing it--- Scaling on the final data ----
```{r}
# player1_final <- as.data.frame(scale(subset(player1, select = -c(PositionGroup))))

# ---- SCALING ----
player_data <- subset(player1, select = -c(PositionGroup))
# calculate the pre-process parameters from the dataset
preprocessParams <- preProcess(player_data, method=c("range"))
preprocessParams
# summarize transform parameters
print(preprocessParams)
# transform the dataset using the parameters
data_scaled <- predict(preprocessParams, player_data) + c(0.0000001)
...
player1_final <- data_scaled

reduce_all_pos_fit = lm(ValueNumeric_pounds ~.-LWB-RWB-LB-CB-RB-CDM-CM-RM-LM-CAM-CF-RF-LF-RW-LW, data=player1_final)
full = lm(ValueNumeric_pounds ~., data=player1_final)
summary(full)
# get rid of aliased coefficients 
reduce_na_fit = lm(ValueNumeric_pounds ~.-RWB-RB-LM-RF-LF-LW, data=player1_final)
```
...
player1_final <- player1 
car::vif(reduce_na_fit)

player1_defend <- subset(player1_without_PositionGroup, select=c("LWB", "LB", "CB", "RB", "RWB"))
cor(player1_defend)

player1_mid <- subset(player1_without_PositionGroup, select=c( "CDM", "CM", "RM", "LM", "CAM"))
cor(player1_mid)

player1_forward <- subset(player1_without_PositionGroup, select=c( "CF", "RF", "LF", "LW", "RW"))
cor(player1_forward)

player1_sub <- subset(player1_without_PositionGroup, select=c( "LWB", "LB", "CB", "RB", "RWB","CDM", "CM", "RM", "LM", "CAM","CF", "RF", "LF", "LW", "RW"))
cor(player1_sub)

player1_x <- subset(player1_without_PositionGroup, select = -c(ValueNumeric_pounds)) 
...
player1_final <- player1_final[, ! names(player1_final) %in% name_col_rem, drop = F]
...
```{r}
plot(reduce_na_fit)
final_fit <- lm(Overall~. , data=player1_final)
summary(final_fit)
plot(final_fit)
```


Run on Overall rating
...
Our version of checking prediction
...
best_fit<-lm(ValueNumeric_pounds ~ Overall + Potential + International.Reputation + 
    ...
    Release.Clause_pounds, data=player1_final)
summary(best_fit)
car::vif(best_fit)
```


Jung's explanation on why 2 and 16 are outliers.


```{r}
best_fit1<-lm(ValueNumeric_pounds ~ Overall + Potential + International.Reputation + 
    Crossing + Finishing + ShortPassing + Volleys + Curve + FKAccuracy + 
    LongPassing + BallControl + Acceleration + Agility + Stamina +  Aggression + Vision + Penalties + StandingTackle  + WageNumeric_pounds + Release.Clause_pounds, data=player1_final)
plot(best_fit1)
...
best_fit2<-lm(log(ValueNumeric_pounds) ~ Overall + Potential + International.Reputation + 
    Crossing + Finishing + ShortPassing + Volleys + Curve + FKAccuracy + 
    LongPassing + BallControl + Acceleration + Agility + Stamina +  Aggression + Vision + Penalties + StandingTackle  + log(WageNumeric_pounds) + log(Release.Clause_pounds), data=player1_final)
plot(best_fit2)
summary(best_fit2)
```
...
```{r}
best_fit3<-lm(ValueNumeric_pounds ~ WageNumeric_pounds+Release.Clause_pounds, data=player1_final)
plot(best_fit3)
summary(best_fit3)
```
